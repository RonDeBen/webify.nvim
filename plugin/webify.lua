...
local function open_file_on_main(opts)
    require('webify').open_file_in_browser_on_main()
end

local function yank_file_url_on_main(opts)
    require('webify').yank_file_url_on_main(opts.fargs[1])
end
...
vim.api.nvim_create_user_command('OpenFileInMain', open_file_on_main, {})
vim.api.nvim_create_user_command('YankFileUrlOnMain', yank_file_url_on_main, { nargs = 1 })
